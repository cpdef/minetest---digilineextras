...
local get_chip_size = function(pos)
	return MEMORY_CHIPS[tonumber(minetest.get_node(pos).name:split("_")[4])]
end
...
local on_digiline_receive = function(pos, node, channel, msg)
	...
	if channel == meta:get_string("channel") then
		if msg == DEL_CMD then
			...
			digiline:receptor_send(pos, digiline.rules.default, channel, OK_MSG)  --here was error: digilines instead of digiline
			return
		end

		if msg:split(" ")[2] == nil then return end -- no arguments :/

		cmd = msg:split(" ")[1]  -- changed [2] to [1]
...
for i, s in ipairs(MEMORY_CHIPS) do
	minetest.register_node("digilines_memory_juli:memory_"..i, {
	...
	if i ~= 1 then
		minetest.register_craft({
			type = "shapeless",
			output = "digilines_memory_juli:memory_"..i,
			recipe = {
				"digilines_memory_juli:memory_"..(i - 1),
				"digilines_memory_juli:memory_"..(i - 1),
			},
		})
	end
